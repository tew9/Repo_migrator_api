...
namespace DevOps.Repo.Client
{
  public class TemplateService : ITemplateService
  {
    ...
    public TemplateService(IClientConfig config)
    {
    ...
    public async Task<HttpResponseMessage> GetTemplate(string templateName)
    {
      try
      {
        var endpoint = $"{_repoApiBaseUrl}/taskmaster/repos/v1/templates/{templateName}";
    ...
    public async Task<HttpResponseMessage> GetAllTemplates()
    {
      try
      {
        var endpoint = $"{_repoApiBaseUrl}/taskmaster/repos/v1/templates";
        var oktaToken = await GetOktaToken();
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", oktaToken);
        _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)");
        var response = await _httpClient.GetAsync(endpoint);
